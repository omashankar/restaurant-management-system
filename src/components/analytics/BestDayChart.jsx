import { bestDayOfWeek } from "@/lib/mockData";
import { CalendarDays } from "lucide-react";

export default function BestDayChart() {
  const best = bestDayOfWeek.reduce((a, b) => (a.score > b.score ? a : b));
        ...
        {bestDayOfWeek.map((d) => {
          ...
          return (
            <div key={d.day} className="flex flex-1 flex-col items-center gap-1.5">
              ...
              <span className={`text-[10px] font-semibold ${isBest ? "text-amber-400" : "text-zinc-500"}`}>
                {d.day}
              </span>
            </div>
          );
