  ...
  if (data.length === 0) {
    return (
      <div className="rounded-2xl border admin-shell-border bg-zinc-900/60 p-5">
        <h3 className="admin-surface-title text-sm font-semibold">Peak Order Times</h3>
  ...
  return (
    <div className="rounded-2xl border admin-shell-border bg-zinc-900/60 p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="admin-surface-title text-sm font-semibold">Peak Order Times</h3>
          <p className="text-xs admin-surface-muted">Hourly order volume</p>
        </div>
        ...
        {data.map((h) => {
          ...
          return (
            <div key={h.hour} className="group flex flex-1 flex-col items-center gap-1">
              ...
              <span className={`text-[9px] font-medium ${isPeak ? "text-indigo-400" : "admin-surface-faint"}`}>{h.hour}</span>
            </div>
          );
        })}
      </div>
