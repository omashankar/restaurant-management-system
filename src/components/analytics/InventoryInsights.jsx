import { INITIAL_INVENTORY_ITEMS } from "@/lib/modulesData";
import { Package, AlertTriangle } from "lucide-react";
import Link from "next/link";

const lowStock = INITIAL_INVENTORY_ITEMS.filter(
  (i) => i.quantity === 0 || i.quantity < i.reorderLevel
);

const mostUsed = [
  { name: "Extra virgin olive oil", usage: 18, unit: "bottle/wk" },
  { name: "Ribeye primal", usage: 14, unit: "kg/wk" },
  { name: "Heirloom tomatoes", usage: 12, unit: "crate/wk" },
  { name: "Parmigiano Reggiano", usage: 6, unit: "wheel/wk" },
];
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-amber-500/25 bg-amber-500/5 p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link href="/inventory" className="cursor-pointer text-xs font-medium text-amber-400 hover:text-amber-300">
            Manage →
          </Link>
        </div>
