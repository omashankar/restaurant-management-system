...
function InstagramIcon({ className = iconCls }) {
  return (
...
function FacebookIcon({ className = iconCls }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden>
...
function TwitterIcon({ className = iconCls }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden>
...
function WhatsAppIcon({ className = iconCls }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden>
...
function YouTubeIcon({ className = iconCls }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden>
...
export function SocialIconGraphic({ id, className = iconCls }) {
  ...
  return <span className="text-[10px] font-bold uppercase">{id?.[0] ?? "?"}</span>;
}

/**
 * @param {{ id: string, label?: string, href: string }[]} links
 * @param {"header"|"footer"|"inline"} variant
 */
export default function SocialMediaIcons({
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`.trim()} role="list">
      {links.map(({ id, label, href }) => (
        <Wrapper
          ...
          {...motionProps}
        >
          <SocialIconGraphic id={id} className={iconClassName ?? iconCls} />
        </Wrapper>
      ))}
    </div>
  );
