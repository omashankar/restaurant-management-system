    ...
    >
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-zinc-950">
        ...
        <span className="menu-card-image-badge menu-card-image-price absolute bottom-2 right-2 rounded-lg px-2.5 py-1 text-sm font-bold tabular-nums text-ra-primary">
          {isPOS
            ? formatMenuPriceRange(item, (n) => formatAdminMoney(n, currency, { decimals: 2 }))
            : itemHasSizes(item)
              ? formatMenuPriceRange(item, (n) => `$${Number(n).toFixed(2)}`)
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        ...
        <div className="flex flex-wrap items-center gap-1">
          ...
          {isPOS && itemHasSizes(item) && (
            <span className="rounded-full bg-sky-500/15 px-2 py-0.5 text-[9px] font-semibold text-sky-300 ring-1 ring-sky-500/25">
              Options
            </span>
          )}
