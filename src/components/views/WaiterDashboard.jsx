  ...
  return (
    <div className="space-y-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {tables.map((t) => (
              <div key={t.id ?? t.tableNumber}
                className={`rounded-2xl border p-4 ${t.status === "occupied" ? "border-sky-500/25 bg-sky-500/5" : "admin-surface-card"}`}>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-lg font-semibold admin-shell-text">{t.tableNumber}</p>
                ...
                <p className="mt-1 text-xs admin-surface-faint">{t.zone ?? ""} · {t.capacity} seats</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Link href="/orders" className="cursor-pointer inline-flex items-center gap-2 rounded-xl border admin-shell-border px-5 py-2.5 text-sm font-semibold admin-shell-text hover-border-ra-primary-40">
          View All Orders
        </Link>
      </div>
