  ...
  return (
    <label className={`flex items-start justify-between gap-3 rounded-xl border admin-shell-border px-3 py-3 sm:gap-4 sm:px-4 ${adminSurface.card}`}>
      <span className="min-w-0 flex-1">
        <span className={`block break-words text-sm font-medium ${adminSurface.title}`}>{label}</span>
        {hint ? <span className={`mt-0.5 block break-words text-xs ${adminSurface.muted}`}>{hint}</span> : null}
      </span>
      <button
        type="button"
        role="switch"
